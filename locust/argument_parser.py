...
def setup_parser_arguments(parser):
    ...
    web_ui_group.add_argument(
        "--web-port", "-P", type=int, default=8089, help="Port on which to run web host"
    )
    ...
    # if we should print stats in the console
    stats_group.add_argument(
        "--print-stats", action="store_true", help="Print stats in the console"
    )
    # only print summary stats
    stats_group.add_argument(
        "--only-summary", action="store_true", help="Only print the summary stats"
    )
    stats_group.add_argument(
        "--reset-stats",
        action="store_true",
    ...
    # log file
    log_group.add_argument(
        "--logfile", help="Path to log file. If not set, log will go to stdout/stderr",
    )
...
def parse_options(args=None):
    parser = get_parser()
    if "LOCUST_MASTER_SERVICE" in os.environ and not isinstance(
        os.environ.get("LOCUST_MASTER_PORT", 0), int
    ):
        sys.stderr.write(
            "Are you running in kubernetes? If you have a container called LOCUST_MASTER, kubernetes will set an env var called LOCUST_MASTER_PORT which will collide with locust's --master-port setting. Please rename your container.\n"
        )
