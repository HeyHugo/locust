...
class LocustRunner(object):
    ...
    def weight_locusts(self, amount):
        ...
        if len(bucket) < amount:
            ...
            for locust in [l for l, r in sorted(residuals.items(), key=lambda x: x[1])][
                : len(bucket) - amount
            ]:
        ...
        while True:
            if not self.cpu_threshold_exceeded:
                ...
                if current_cpu > 90:
                    ...
                    logging.warning(
                        "Loadgen CPU usage above 90%! This may constrain your throughput and even give inconsistent response time measurements! See https://docs.locust.io/en/stable/running-locust-distributed.html for how to distribute the load over multiple CPU cores or machines"
                    )
            gevent.sleep(2.0)
        ...
        while True:
            ...
            if msg.type == "client_ready":
                ...
                if msg.node_id in self.clients:
                    ...
                    if (
                        not c.cpu_threshold_exceeded
                        and msg.data["cpu_threshold_exceeded"]
                    ):
                        c.cpu_threshold_exceeded = True
                        self.slave_cpu_threshold_exceeded = True
                        logger.warning(
                            "Slave %s exceeded cpu threshold" % (msg.node_id)
                        )
            elif msg.type == "stats":
            ...
            self.client.send(
                Message(
                    "heartbeat",
                    {
                        ...
                        "cpu_threshold_exceeded": self.cpu_threshold_exceeded,
                    },
                    self.client_id,
                )
            )
