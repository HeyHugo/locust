...
def parse_options(args=None, default_config_files=["~/.locust.conf", "locust.conf"]):
    ...
    parser.add_argument(
        "-P", "--web-port", type=int, default=8089, help="Port on which to run web host"
    )
...
def main():
    ...
    environment = Environment(locust_classes=locust_classes, options=options,)
    ...
    if options.no_web:
        ...
        main_greenlet = gevent.spawn(
            environment.web_ui.start, host=options.web_host, port=options.web_port
        )
