...
class TestWriteStatCSVs(LocustTestCase):
    ...
    def setUp(self):
        ...
        locust.runners.locust_runner = locust.runners.LocalLocustRunner(
    ...
    def test_write_csv_files_full_history(self):
        locust.stats.write_csv_files(
            self.runner.stats, self.STATS_BASE_NAME, full_history=True
        )
        self.assertTrue(os.path.exists(self.STATS_FILENAME))
        self.assertTrue(os.path.exists(self.STATS_HISTORY_FILENAME))
        self.assertTrue(os.path.exists(self.STATS_FAILURES_FILENAME))
    ...
    def test_csv_stats_writer(self):
        with mock.patch("locust.stats.CSV_STATS_INTERVAL_SEC", new=0.2):
            greenlet = gevent.spawn(
                stats_writer, self.runner.stats, self.STATS_BASE_NAME
            )
            gevent.sleep(0.21)
    ...
    def test_csv_stats_writer_full_history(self):
        ...
        with mock.patch("locust.stats.CSV_STATS_INTERVAL_SEC", new=0.2):
            greenlet = gevent.spawn(
                stats_writer, self.runner.stats, self.STATS_BASE_NAME, full_history=True
            )
            gevent.sleep(0.21)
            gevent.kill(greenlet)
            self.assertTrue(os.path.exists(self.STATS_FILENAME))
    ...
    def test_csv_stats_on_master_from_aggregated_stats(self):
        ...
        with mock.patch("locust.rpc.rpc.Server", mocked_rpc()) as server:
            master = MasterLocustRunner(
                self.environment, [], master_bind_host="*", master_bind_port=0
            )
            ...
            locust.stats.write_csv_files(
                master.stats, self.STATS_BASE_NAME, full_history=True
            )
            self.assertTrue(os.path.exists(self.STATS_FILENAME))
            self.assertTrue(os.path.exists(self.STATS_HISTORY_FILENAME))
            self.assertTrue(os.path.exists(self.STATS_FAILURES_FILENAME))
...
class TestStatsEntry(unittest.TestCase):
    ...
    def test_fail_ratio_with_no_failures(self):
        ...
        self.assertAlmostEqual(
            output_fields["failure_precentage"], EXPECTED_FAIL_RATIO * 100
        )
