...
class MovingAverageTest(WebserverTestCase):
    ...
    def _deviation(self, taskset, percentage):
        ...
        print "Deviation: %.3f ms (%1.3f%%), max: %s ms (%s%%)" % (
            deviation,
            deviation / taskset.avg_wait * 100.0,
            max_deviation,
            percentage,
        )
    ...
    def _dump_stats(self, taskset):
        print "Num waits: %d Wanted Average: %s Actual Average: %s" % (
            taskset._avg_wait_ctr,
            taskset.avg_wait,
            taskset._avg_wait,
        )
    ...
    def _assertion(self, taskset, deviation, max_deviation, percentage):
        ...
        self.assertTrue(
            ...
            msg="Deviation not within %s%% of wanted average" % percentage,
        )

    def test_moving_average_100000(self):
        taskset = self.get_task_set(
            3000, 140000, 20 * 60 * 1000
        )  # 3 seconds, 140 seconds, 20 minutes
    ...
    def test_omit_average(self):
        taskset = self.get_task_set(
            3000, None, 20 * 60 * 1000
        )  # 3 seconds, None, 20 minutes
