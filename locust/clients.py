...
def log_request(f):
    def _wrapper(*args, **kwargs):
        ...
        try:
            ...
            if delayed:
                retval.delay_success = lambda: events.request_success.fire(
                    name, response_time, retval
                )
                retval.delay_failure = lambda e: events.request_failure.fire(
                    name, response_time, e, None
                )
            else:
            ...
            events.request_failure.fire(
                name, response_time, e, response=e.locust_http_response
            )
...
class HttpResponse(object):
    ...
    def _get_data(self):
        if (
            self._gzip
            and not self._decoded
            and self._info().get("Content-Encoding") == "gzip"
        ):
    ...
    def __enter__(self):
        if not self.delayed:
            raise LocustError(
                "If using response in a with statement you must use delayed=True"
            )
        return self
...
class HttpBrowser(object):
    ...
    def _request(self, path, data=None, headers={}, name=None):
        ...
        try:
            ...
            data = e.read()
            e.locust_http_response = HttpResponse(
                url, name, e.code, data, e.info, self.gzip
            )
            e.close()
            raise e
