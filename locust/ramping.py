...
def current_percentile(percent):
    if is_distributed:
        ...
        return percentile(
            sorted([item for sublist in response_times for item in sublist]), percent
        )
    else:
...
def on_request_success_ramping(_, _1, response_time, _2):
    if is_distributed:
        ...
        if len(response_times) > rps * PERCENTILE_TIME_WINDOW:
            for i in xrange(
                len(response_times) - int(math.ceil(rps * PERCENTILE_TIME_WINDOW))
            ):
        ...
        while True:
            if locust_runner.state != STATE_HATCHING:
                if locust_runner.num_clients >= max_locusts:
                    logger.info(
                        "Ramp up halted; Max locusts limit reached: %d" % max_locusts
                    )
                ...
                if fail_ratio > acceptable_fail:
                    logger.info(
                        "Ramp up halted; Acceptable fail ratio %d%% exceeded with fail ratio %d%%"
                ...
                if p >= response_time_limit:
                    logger.info(
                        "Ramp up halted; Percentile response times getting high: %d" % p
                    )
                    return ramp_down(clients, hatch_stride)

                if boundery_found and hatch_stride <= precision:
                    logger.info(
                        "Sweet spot found! Ramping stopped at %i locusts"
        ...
        while True:
            if locust_runner.state != STATE_HATCHING:
                    ...
                    if fail_ratio <= acceptable_fail:
                        ...
                        if p <= response_time_limit:
                            ...
                            locust_runner.start_hatching(
                                clients, locust_runner.hatch_rate
                            )
                ...
                if clients > 0:
                    ...
                    logger.warning(
                        'No responses met the ramping thresholds, check your ramp configuration, locustfile and "--host" address'
                    )
