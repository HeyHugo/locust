...
class RequestStats(object):
    ...
    def current_rps(self):
        ...
        slice_start_time = max(
            self.global_last_request_timestamp - 12, int(self.global_start_time or 0)
        )

        reqs = [
            self.num_reqs_per_sec.get(t, 0)
            for t in range(slice_start_time, self.global_last_request_timestamp - 2)
        ]
        return avg(reqs)
    ...
    def add_stats(self, other, full_request_history=False):
        ...
        if full_request_history:
            new.response_times = merge_dict_add(
            ...
            for i in xrange(
                new.last_request_timestamp - 20, new.last_request_timestamp + 1
            ):
                new.num_reqs_per_sec[i] = self.num_reqs_per_sec.get(
                    i, 0
                ) + other.num_reqs_per_sec.get(i, 0)
...
def on_slave_report(client_id, data):
    for stats in data["stats"]:
        ...
        RequestStats.requests[stats.name] = RequestStats.requests[stats.name].add_stats(
            stats, full_request_history=True
        )
...
def print_percentile_stats(stats):
    ...
    if complete_list:
        print (
            " %-" + str(STATS_NAME_WIDTH) + "s %8s %6d %6d %6d %6d %6d %6d %6d %6d %6d"
        ) % (
            "Total",
