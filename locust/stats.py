    ...
    def on_slave_report(client_id, data):
        for stats_data in data["stats"]:
            ...
            if not request_key in environment.stats.entries:
                environment.stats.entries[request_key] = StatsEntry(
                    environment.stats, entry.name, entry.method
                )
        ...
        if (
            environment.stats.total.last_request_timestamp
            and environment.stats.total.last_request_timestamp
            ...
            environment.stats.total._cache_response_times(
                int(environment.stats.total.last_request_timestamp)
            )
