...
def tag(tag_name=None):
    ...
    def decorator_func(decorated):
        if issubclass(type(decorated), TaskSetMeta):
            ...
            if (
                "locust_tag_union" not in decorated.__dict__
                or "locust_tag_intersection" not in decorated.__dict__
            ):
                decorated.locust_tag_union = decorated.locust_tag_intersection = set(
                    ["tagged"]
                )
...
class TaskSet(object, metaclass=TaskSetMeta):
    ...
    def __init__(self, parent):
        ...
        if isinstance(parent, TaskSet):
            ...
            raise LocustError(
                "TaskSet should be called with User instance or TaskSet instance as first argument"
            )
